...
// Cart module to handle cart operations
class Cart {
    constructor() {
    ...
    addItem(itemData) {
        ...
        const existingItem = this.cart.find(item => item.id === itemData.id);
    ...
    getCartTotal() {
        return this.cart.reduce((total, item) => {
            ...
            const brandingTotal = item.branding ? 
                item.branding.reduce((sum, b) => sum + (b.price * item.quantity), 0) : 0;
            return total + itemTotal + brandingTotal;
        }, 0);
    }
    ...
    showSuccessNotification() {
        ...
        notification.innerHTML = `
            ...
            <div class="success-notification__content">
                ...
                <p class="success-notification__message">Товар успешно добавлен в вашу корзину</p>
            </div>
    ...
    // Bind event listeners for all product pages
    bindEvents() {
        document.addEventListener('click', (e) => {
            ...
            // Check if button is in a product container
            const productContainer = e.target.closest('.product, .detail-page__content');
            if (!productContainer) return;

            // Get selected color
            const selectedColorEl = productContainer.querySelector('.color-label__check[style*="display:block"]');
            const selectedColor = selectedColorEl ? {
                id: selectedColorEl.dataset.id,
                name: selectedColorEl.closest('.color-label').querySelector('.tooltip-text').textContent,
                hex: selectedColorEl.closest('.color-label').querySelector('.square').style.backgroundColor
            } : null;

            // Собираем данные о товаре
            const itemData = {
                id: productContainer.dataset.id,
                article: productContainer.querySelector('.article').textContent.trim(),
                name: productContainer.querySelector('h3').textContent.trim(),
                price: parseFloat(productContainer.querySelector('.price').textContent.replace(/[^\d.]/g, '')),
                quantity: 1,
                selectedColor: selectedColor,
                image: productContainer.querySelector('img')?.src || '/static/viki_web/icons/logo.svg',
                description: productContainer.querySelector('.product-hor__color-description')?.textContent.trim(),
                branding: []
            };

            // Добавляем брендирование если есть
            const brandingSection = productContainer.querySelector('.cart-item__branding');
            if (brandingSection) {
                const brandingItems = brandingSection.querySelectorAll('.branding-item');
                itemData.branding = Array.from(brandingItems).map(item => ({
                    type: item.querySelector('.branding-type').value,
                    location: item.querySelector('.branding-location').value,
                    colors: parseInt(item.querySelector('.branding-colors').value),
                    secondPass: item.querySelector('.branding-second-pass').checked,
                    price: parseFloat(item.querySelector('.branding-price').value)
                }));
            }
