...
import { initModalDnD } from './common/modalDnD.js';
import { showManagerInfo } from './order_list/managerModal.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Показать инфо о менеджере
    document.querySelectorAll('.order-button_manager').forEach(button => {
        button.addEventListener('click', (e) => {
            e.preventDefault();
            const name = button.dataset.managerName;
            const email = button.dataset.managerEmail;
            const phone = button.dataset.managerPhone;
            showManagerInfo(name, email, phone);
        });
    });
